...
class TankSensor(Sensor):
	def __init__(self, monitor: 'Monitor', name: str, pin: int, 
				 when_submerged: List[str], when_exposed: List[str]):
		...
		when_submerged : List[str]
			A list of tank states to report as True when submerged,
			otherwise reports False
		when_exposed : List[str]
			A list of tank states to report as True when exposed,
			otherwise reports False
		"""
	...
	def _update_state_vals(self, set_true: List[str], set_false: List[str]):
		...
		----------
		set_true : List[str]
		  A list of tank states to report a True contribution to the monitor,
		  if all other sensors report True to the monitor for this state, it
		  will be eligible to be considered active.
		set_false : List[str]
...
class ErrorSensor(Sensor):
	def __init__(self, monitor: 'Monitor', name: str, pin: int,
				 trigger_when: str, permitted_runs: int):
		"""A class to represent an error sensor which has the potential to
		terminate routine steps early based on an alloted number of remaining
		runs. An example is a sensor which senses when an RODI reservoir is low.
        ...
        pin : int
            The broadcom (BCM) pin number for the sensor
		trigger_when : str
			Whether the sensor should report errors when 'submerged' or
			'exposed'
		permitted_runs : int
...
class Monitor:
	def __init__(self, sensors: Dict[str, Union[TankSensor, ErrorSensor]] = {},
				 tank_states: Dict[str, Dict[str, List[bool]]] = {},
		...
		their respective contribution(s) t
		Parameters
		----------
		sensors : Dict[str, Union[TankSensor, ResevoirSensor]], optional
			A dictionary of sensor names (as strings) as keys and instances of
			objects inheriting from :class: Sensor, by default {}
		tank_states : Dict[str, List[bool]]], optional
			A dictionary of possible tank states (as strings) as high-level keys
			and a dictionary of lower-level keys given by all the sensors that
			affect that state. The values are interpreted indirectly, by
			:property: tank_state. By default, {}
		error_checks : Dict[str, bool], optional
			A dictionary of error check names (as strings) as keys and whether
			the associated sensor is actively triggered. The values are
			interpreted indirectly by :meth: check_error. By default, {}
		"""
	...
	def register(self, sensor_type: str, name: str = None, pin: int = None,
				 **kwargs: dict) -> Union[TankSensor, ErrorSensor]:
		"""Instantiates a new object inheriting from :class: Sensor and stores
		it in the sensors attibute dictionary.
		Parameters
		----------
		sensor_type : str
			The type of sensor to instantiate. Primarily used as a validation
			method to ensure that appropriate arguments are passed.
			Can be either 'error' or 'tank'
        name : str
            A descriptive name for the sensor
        pin : int
            The broadcom (BCM) pin number for the sensor
		kwargs : dict
			All of the arguments which are specific to one sublass of Sensor or
			the other.
		...
		try:
			if sensor_type == 'tank':
				_type = TankSensor
				state_set = kwargs['when_submerged'] + kwargs['when_exposed']
			...
			sensor = _type(monitor = self, name = name, pin = pin, **kwargs)
	...
	def tank_state(self) -> Union[str, CheckError]:
		...
		tank_state : Union[str, CheckError]
			A string representing the tank state. If a CheckError is returned,
			then no valid tank state was found. In most cases this indicates
			either improper configuration or a malfunctioning sensor. If this
			is reported during a routine, the program will generally notify all
			error contacts for that routine.
		"""
		states = {}
		for state, records in self.tank_states.items():
			states[state] = all(records.values())

		if sum(states.values()) == 1:
			states = [k for k,v in states.items() if v]
			return states[0]
		else:
			return CheckError()

	def check_error(self, name: str, decrement:
					bool = True) -> Union[bool, ErrorSensorTriggered]:
		...
		decrement : bool, optional
			Whether to decrease the remaining_runs (only relevant if
			triggered), by default True

		Returns
		-------
		Union[bool, ErrorSensorTriggered]
			Either False (no error) or the details of the error triggered
			(evaluates as True)
		"""
