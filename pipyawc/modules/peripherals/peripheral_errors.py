...
class SensorInstanceError(PeripheralInstanceError):
    ...
    def expected_kwargs(self) -> Dict[str, str]:
        if self._classname == "TankSensor":
            kwargs = {"when_submerged": "List[str]", "when_exposed": "List[str]"}
        elif self._classname == "ReservoirSensor":
            kwargs = {"trigger_when": "str", "permitted_runs": "int"}
        return kwargs
...
class PumpTimeoutError(Exception):
    ...
    def __str__(self):
        ...
        return lbl + f" Maximum allowed duration is {self.run_time:.2f} seconds."
